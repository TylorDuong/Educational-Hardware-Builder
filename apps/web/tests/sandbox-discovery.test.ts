...
test("Dashboard renders validated discovery results and preserves the existing five-tab navigation", async () => {
  ...
  assert.match(source, /function PartsPanel\(\{ discovery, ownedParts, onOpenWorkshop \}/);
  assert.match(source, /CURRENT PARTS \(OPTIONAL\)/);
  assert.match(source, /function ReportedOwnedParts/);
  assert.match(source, /Recognized for planning/);
  ...
  assert.match(source, /CACHED_INVENTORY_&amp;_SOURCING/);
  ...
  assert.match(source, /Suggested alternate parts/);
  assert.match(source, /Suggested alternate parts/);
  assert.match(source, /View on \{offer\.provider\}/);
  ...
  assert.match(source, /All assembly parts/);
  assert.match(source, /Selected part/);
